...
%consistent arabic page numbering
\makeatletter
...
\definecolor{niceBlue}{HTML}{035e7f}
...
% info
\author{Sylvain Kern}
\title{This is \texttt{tufte-style-thesis}, a Tufte-styled \LaTeX{} class for theses}
...
% \person{}{jury 2}{\dots}
\logoone{example-image-a}
\logotwo{example-image-b}
% \logothree{example-image-c}
...
\todo{%
Maketitle -> titlepage.
Régler les problèmes de person. Faire que on peut utiliser la commande person plusieurs fois de suite et ça ajoute à la liste des personnes à mettre dans le tableau. France.
}
...
shoutouts to my man \textcolor{niceRed}{Tufte}.
...
Maybe the most distinctive aspect of this class is its page layout with its big margin to put sidenotes and captions. However this is not original at all: plenty\sidecite{} of other \LaTeX{} classes for books and theses do it just like me, and almost always better\sidenote{Or in a way cleaner \LaTeX{}.} I just wanted to do my own thing here, mixing what I personally like the most in these layout types, to better learn \LaTeX{} and to really internalize this kind of design. At the end of the day it may be a more Bringhursty than Tuftey kind of look, but hey, I won't change the name of this whole thing now.
...
\section{The philosophy behind the design}

As a disclaimer: I do not have neither the experience nor the pretention to make authoritary statements about design philosphy\index{design}, these are just some thoughts I gathered that I find interesting to consider when making designs, closely or remotely.
...
\begin{itemize}
\item
\inlinecode{latex}{<number>} is an optional parameter for the sidenote number. For example, \inlinecode{latex}_\sidenote[29100][]{The sidenote.}_ does this.\sidenote[29100][]{The sidenote.}
\item
\inlinecode{latex}{<offset>} is an offset length (in pt, px, en, em\dots) to vertically offset the sidenote. A positive value will have it go down, a negative go up.
\end{itemize}

\LaTeX{} natively allows to put unformatted content in the margin with the command \inlinecode{latex}_\marginpar{<your content>}_,\marginpar{This is unformatted margin text, in fullsize.} but I advise not to use it, as it puts raw fullsize text in the margin, and does not blend well with the overall design. Instead, use\sidetext{This is just some unnumbered piece of text in the margin, but with the formatting done right.}
\begin{codebox}{latex}
  \sidetext{<your text>}
\end{codebox}
...
\begin{wide}
Sometimes, it can be handy to have the text span the whole page width, like this paragraph. Use the environment \inlinecode{latex}_\begin{wide}...\end{wide}_ to do this. It should manage page breaks properly, but it is not optimal: no not use it for too long (like for ten pages), the behavior tends to go a little wild. The behavior of \inlinecode{latex}{\sidenote}, \inlinecode{latex}{\marginpar} and \inlinecode{latex}{\sidecite} is not supported in the \inlinecode{latex}{wide} environment.

Also, for floating environments, full width figures and tables will be covered in the chapter \ref{chp:floats}, so do not use the \inlinecode{latex}{wide} environments with figures or tables (actually tables are fine, but there are specific environments for them to be in full width).
\end{wide}
...
The \inlinecode{latex}_\maketitle_ macro has been slightly pimped up. It now displays a custom titlepage ---like the one on this very document, as well as a copyright tag, a dedication word and a colophon.
...
\todo{
    faire la liste des packages par exemple
}
...
\chapter{Another chunk of maybe completely pointless content}
